...
class AuditServiceTest {
    ...
    fun `the sub is taken from the MDC when the trace filter already set it`() {
        ...
        service.record("courts", 1L, AuditAction.INSERT, newValues = "name=Ana")

        val captor = argumentCaptor<AuditLog>()
        verify(repository).save(captor.capture())
        assertEquals("courts", captor.firstValue.entityName)
    ...
    fun `without MDC the sub falls back to the token`() {
        ...
        service.record("courts", 2L, AuditAction.UPDATE, oldValues = "name=Ana", newValues = "name=Ana Lopez")
    ...
    fun `a token without username claim is audited as anonymous`() {
        ...
        service.record("courts", 3L, AuditAction.DELETE, oldValues = "name=Ana")
    ...
    fun `without authentication everything is audited as anonymous`() {
        service.record("courts", 4L, AuditAction.DELETE)
    ...
    fun `long values are truncated so the audit table never explodes`() {
        ...
        service.record("courts", 5L, AuditAction.UPDATE, oldValues = huge, newValues = huge)
